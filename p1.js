...
let move = (servo, position, angle, max_vel=0) => {
	if(max_vel == 0){
		...
		let movement = setInterval(fu, speed)
		function fu(){
			if(position == angle){
...
setTimeout(goDown, 2000)
setTimeout(goUp, 4000)

setTimeout(goDown, 6000)
setTimeout(goUp, 8000)
...
for(let i = 0; i<15; i++){
	delay = delay + 500
	setTimeout(goUp, delay)
	delay = delay + 500
	setTimeout(goDown, delay)
}

//setTimeout(goUp, 10000)
